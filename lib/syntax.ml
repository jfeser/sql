...
let values_or_all table names =
  ...
  match names with Some names -> Schema.project names schema | None -> schema
...
let rec get_params_q (e : expr_q) =
  let rec loop acc e =
    ...
    | `Choice (p, l) ->
        Choice
          (p, List.map (fun (n, e) -> Simple (n, Option.map get_params_q e)) l)
        :: acc
  in
  loop [] e |> List.rev

let rec is_grouping = function
  | Value _ | Param _ | Column _ | Select _ | Inserted _ -> false
  | Choices _ -> fail "is_grouping Choice TBD"
  | Fun (func, args) ->
      (* grouping function of zero or single parameter or function on grouping result *)
      (Type.is_grouping func && List.length args <= 1)
      || List.exists is_grouping args

let exists_grouping columns =
  List.exists
    (function Expr (e, _) -> is_grouping e | All | AllOf _ -> false)
    columns
...
let get_columns_schema tables l =
  ...
  l
  |> List.map (fun col ->
         { (resolve_column tables all col) with name = col.cname })
...
let rec resolve_columns env expr =
  if !debug then (
    eprintf "\nRESOLVE COLUMNS %s\n%!" (expr_to_string expr);
    eprintf "schema: ";
    Sql.Schema.print env.joined_schema;
    Tables.print stderr env.tables );
  let rec each e =
    match e with
    | Value x -> `Value x
    | Column col ->
        `Value (resolve_column env.tables env.joined_schema col).domain
    | Inserted name ->
        let attr =
          try Schema.find env.insert_schema name
          with Schema.Error (_, s) -> fail "for inserted values : %s" s
        in
        `Value attr.domain
    | Param x -> `Param x
    | Choices (n, l) ->
        `Choice (n, List.map (fun (n, e) -> (n, Option.map each e)) l)
    | Fun (r, l) -> `Func (r, List.map each l)
    | Select (select, usage) -> (
        let as_params p =
          List.map
            (function
              | Single p -> `Param p | Choice _ -> fail "FIXME as_params")
            p
        in
        ...
        | s, `AsValue ->
            raise
              (Schema.Error
                 ( s,
                   "only one column allowed for SELECT operator in this \
                    expression" ))
...
and assign_types expr =
  let option_split = function
    | None -> (None, None)
    | Some (x, y) -> (Some x, Some y)
  in
  let rec typeof (e : expr_q) =
    (* FIXME simplify *)
    match e with
    | `Value t -> (e, `Ok t)
    | `Param (_, t) -> (e, `Ok t)
    | `Choice (n, l) ->
        let e, t =
          List.split
          @@ List.map (fun (_, e) -> option_split @@ Option.map typeof e) l
        in
        let t =
          ...
          | t :: ts ->
              List.fold_left
                (fun acc t ->
                  match acc with
                  | None -> None
                  | Some prev -> Type.common_subtype prev t)
                (Some t) ts
        in
        let t =
          match t with
    ...
    | `Func (func, params) ->
        ...
        let show () =
          sprintf "%s applied to (%s)" (string_of_func func)
        ...
        let func =
          ...
          | x -> x
        in
        let ret, inferred_params =
          match (func, types) with
          | Multi _, _ -> assert false (* rewritten into F above *)
          | Agg, [ typ ] | Group typ, _ -> (typ, types)
          | Agg, _ ->
              fail "cannot use this grouping function with %d parameters"
                (List.length types)
          | F (_, args), _ when List.length args <> List.length types ->
...
and resolve_types env expr =
  ...
  try
    ...
    if !debug then
      eprintf "resolved types %s : %s\n%!" (show_expr_q expr')
        (Type.to_string @@ get_or_failwith t);
    r
  with exn ->
  ...
  let resolve1 = function
    ...
    | Expr (e, name) ->
        let col =
          ...
          | _ ->
              make_attribute ""
                (resolve_types env e |> snd |> get_or_failwith)
                Constraints.empty
        in
        let col =
          Option.map_default (fun n -> { col with name = n }) col name
        in
        [ col ]
  in
...
and params_of_columns env =
  let get = function All | AllOf _ -> [] | Expr (e, _) -> get_params env e in
  flat_map get

and get_params_opt env = function Some x -> get_params env x | None -> []
...
and do_join (env, params) ((schema1, params1, _tables), kind) =
  let joined_schema =
    match kind with
    | `Cross | `Search _ | `Default -> Schema.cross env.joined_schema schema1
...
and join env ((joined_schema, p0, ts0), joins) =
  ...
  let all_tables =
    List.flatten (ts0 :: List.map (fun ((_, _, ts), _) -> ts) joins)
  in
    ...
    List.filter_map
      (function
        ...
        | Some (`Param p) ->
            Some
              (Choice
                 (p, [ Verbatim ("ASC", "ASC"); Verbatim ("DESC", "DESC") ])))
      directions
  in
  get_params_l
    {
      tables;
      joined_schema = final_schema :: List.map snd tables |> all_columns;
      insert_schema = [];
    }
    orders
  @ directions
...
and ensure_simple_expr = function
  ...
  | Fun (func, _) when Type.is_grouping func ->
      failwith "Grouping function not allowed in simple expression"
...
and eval_nested env nested =
  ...
  | Some (t, l) ->
      join env
        ( resolve_source env t,
          List.map (fun (x, k) -> (resolve_source env x, k)) l )
  | None -> (env, [])
...
and eval_select_full env { select = select, other; order; limit } =
  ...
  let s2l, p2l =
    List.split
      ( List.map (fun (s, p, _, _) -> (s, p))
      @@ List.map (eval_select env) other )
  in
  if false then
  ...
  let p4, limit1 =
    match limit with
    | Some (p, x) -> (List.map (fun p -> Single p) p, x)
    | None -> ([], false)
  in
  (*                 Schema.check_unique schema; *)
  let cardinality =
    if limit1 && cardinality = `Nat then `Zero_one else cardinality
  in
  ( final_schema,
    (p1 @ List.flatten p2l @ p3 @ p4 : var list),
    Stmt.Select cardinality )
...
let update_tables sources ss w =
  ...
  let tables = List.flatten @@ List.map (fun (_, _, ts) -> ts) sources in
  (* TODO assert equal duplicates if not unique *)
  let env =
    {
      tables;
      joined_schema;
...
let annotate_select select types =
  ...
  let rec loop acc cols types =
    ...
    | Expr (e, name) :: cols, t :: types ->
        loop (Expr (Fun (F (Typ t, [ Typ t ]), [ e ]), name) :: acc) cols types
    | _, [] | [], _ -> failwith "Select cardinality doesn't match Insert"
  in
  {
    select with
    select =
      ({ select1 with columns = loop [] select1.columns types }, compound);
  }
...
let eval (stmt : Sql.stmt) =
      ...
      List.iter
        (function
          ...
          | `Change (oldcol, col, pos) ->
              Tables.alter_change name oldcol col pos
  ...
  | Rename l ->
      ...
      ([], [], Alter (String.concat "," @@ List.map fst l))
      (* to have sensible target for gen_xml *)
  | Drop name ->
      Tables.drop name;
      ([], [], Drop name)
  | CreateIndex (name, table, cols) ->
      Sql.Schema.project cols (Tables.get_schema table) |> ignore;
  ...
  | Insert { target = table; action = `Values (names, values); on_duplicate }
    ->
      ...
      let env =
        {
          tables = [ Tables.get table ];
          joined_schema = Tables.get_schema table;
  ...
  | Insert { target = table; action = `Select (names, select); on_duplicate }
    ->
      ...
      in
      let select =
        annotate_select select (List.map (fun a -> a.domain) expect)
      in
      let schema, params, _ = eval_select_full env select in
      ignore (Schema.compound expect schema);

      (* test equal types once more (not really needed) *)
      let params2 = params_of_assigns env (Option.default [] on_duplicate) in
  ...
  | Delete (table, where) ->
      ...
      let p =
        get_params_opt
          { tables = [ t ]; joined_schema = snd t; insert_schema = [] }
          where
      in
      ([], p, Delete table)
  | Set (_name, e) ->
      let p =
        match e with
        | Column _ ->
            [] (* this is not column but some db-specific identifier *)
...
let unify_params l =
  ...
  let check_choice_name = function
    | None -> () (* unique *)
    | Some n when Hashtbl.mem h_choices n ->
        fail "sharing choices not implemented"
  ...
  let remember name t =
    ...
    | Some name -> (
        ...
        | t' -> (
            ...
            | None ->
                fail "incompatible types for parameter %S : %s and %s" name
                  (Type.show t) (Type.show t') ) )
  in
  let rec traverse = function
    | Single ((name, _loc), t) -> remember name t
    | Choice (n, l) ->
        check_choice_name (fst n);
        List.iter
          (function
            | Simple (_, l) -> Option.may (List.iter traverse) l
            | Verbatim _ -> ())
          l
  in
  let rec map = function
    | Single (((None, _), _) as x) -> Single x
    | Single (((Some name, _) as id), _) ->
        Single (id, try Hashtbl.find h name with _ -> assert false)
    | Choice (id, l) ->
        Choice
          ( id,
            List.map
              (function
                | Simple (n, l) -> Simple (n, Option.map (List.map map) l)
                | Verbatim _ as v -> v)
              l )
  in
...
let common_prefix = function
  ...
  | x :: _ as l ->
      let rec loop i =
        ...
        else if List.for_all (fun s -> i < String.length s && s.[i] = x.[i]) l
        then loop (i + 1)
        else i
      in
      let i = loop 0 in
      (* do not allow empty names or starting not with alpha *)
      if List.exists (fun s -> i = String.length s || not (is_alpha s.[i])) l
      then 0
      else i
...
let complete_sql kind sql =
  ...
  | Stmt.Insert (Some (kind, schema), _) ->
      ...
      let params = ref [] in
      let first =
        common_prefix @@ List.map (fun attr -> attr.Sql.name) schema
      in
      schema
      |> List.iter (fun attr ->
             ...
             let param =
               Single ((Some attr_name, (pos_start, pos_end)), attr.Sql.domain)
             in
