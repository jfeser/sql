...
let xml_to_string xml =
  ...
  let rec iter spaces = function
    | Node (name, attrs, children) -> (
        ...
        | _ ->
            bprintf b ">";
            List.iter (iter (spaces ^ " ")) children;
            bprintf b "\n%s</%s>" spaces name )
    | Comment text ->
        bprintf b "\n<!-- %s -->"
...
let params_to_values =
  List.map (fun (n, t) -> value n t) $ all_params_to_values
...
let generate_code (x, _) index stmt =
  ...
  let attrs =
    ...
    | Insert (_, t) ->
        [ ("kind", "insert"); ("target", t); ("cardinality", "0") ]
    | Create t -> [ ("kind", "create"); ("target", t); ("cardinality", "0") ]
    | CreateIndex t ->
        [ ("kind", "create_index"); ("target", t); ("cardinality", "0") ]
    | Update None -> [ ("kind", "update"); ("cardinality", "0") ]
    | Update (Some t) ->
        [ ("kind", "update"); ("target", t); ("cardinality", "0") ]
  ...
  in
  x :=
    Node ("stmt", ("name", name) :: ("sql", sql) :: attrs, [ input; output ])
    :: !x

let start_output (x, pre) =
  pre := !x;
  x := []
