...
let comment_doc sl =
  output "/**";
  output_l
    (List.map (fun str -> Gen_caml.replace_all ~sub:"*/" ~by:"* /" ~str) sl);
  output "*/"

let comment_xml summary params =
  let summary = String.nsplit summary "\n" in
  let params =
    List.map (fun (n, s) -> sprintf "<param name=\"%s\">%s</param>" n s) params
  in
...
let get_column attr index =
  sprintf "reader.Get%s(%u)" (attr.domain |> as_api_type) index
...
let set_param index param =
  ...
  output "%s.DbType = DbType.%s;" name t;
...
let func_execute index stmt =
  ...
  let doc =
    if is_select then [ ("result", schema_to_string stmt.schema) ] else []
  in
  comment_xml "execute query" doc;
  let func_name = if is_select then "execute_reader" else "execute" in
  let result =
    "public " ^ if is_select then "IEnumerable<IDataReader>" else "int"
  in
  G.func result func_name values (fun () ->
      ...
      G.close_curly "";
      output
        "if (null != CommandTimeout) _cmd.CommandTimeout = \
         CommandTimeout.Value;";
      List.iteri
        (fun i (name, _) ->
          output "((IDbDataParameter)_cmd.Parameters[%u]).Value = %s;" i name)
        values;
  ...
  if is_select then (
    ...
    G.func "public int" "execute" all_params (fun () ->
        let args =
          List.mapi (fun index attr -> get_column attr index) stmt.schema
        in
        output "int count = 0;";
        output "foreach (var reader in execute_reader(%s))"
    ...
    | _ ->
        ...
        List.iteri
          (fun index attr ->
            output "public readonly %s %s;" (as_lang_type attr.domain)
              (name_of attr index))
          stmt.schema;
        empty_line ();
        G.func "public" "row" [ ("reader", "IDataReader") ] (fun () ->
            List.iteri
              (fun i attr ->
                output "%s = %s;" (name_of attr i) (get_column attr i))
              stmt.schema);
        end_class "row";
