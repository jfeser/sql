...
let parse_one' (sql, props) =
  ...
  ( match (kind, !Gen.params_mode) with
  | Insert (Some _, _), None ->
...
let () =
  Printexc.(
    register_printer (function
      | With_backtrace (exn, bt) when raw_backtrace_length bt = 0 ->
          Some (to_string exn)
      | With_backtrace (exn, bt) ->
          Some
            (Printf.sprintf "%s\nBacktrace:\n%s" (to_string exn)
...
let parse_one ((sql, props) as x) =
  ...
  | Parser_utils.Error (exn, (line, cnum, tok, tail)) ->
      ...
      in
      Error.log "==> %s" sql;
      if cnum = String.length sql && tok = "" then Error.log "Error: %s" extra
      else
        Error.log "Position %u:%u Tokens: %s%s\nError: %s" line cnum tok
          (String.slice ~last:32 tail)
          extra;
      None
  | exn ->
      let bt = Printexc.get_raw_backtrace () in
      Error.log "Failed %s: %s"
        (Option.default "" @@ Props.get props "name")
        sql;
    ...
    | Some sql -> (
        ...
        | Some stmt ->
            if not (Sql.Schema.is_unique stmt.schema) then
              Printf.eprintf
                "Warning: this SQL statement will produce rowset with \
