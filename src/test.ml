...
let cmp_params p1 p2 =
  try
    List.for_all2
      (fun ((name1, pos1), t1) ((name2, pos2), t2) ->
...
let parse sql =
  ...
  | exception exn ->
      assert_failure @@ sprintf "failed : %s : %s" (Printexc.to_string exn) sql
...
let do_test sql ?kind schema params =
  ...
  assert_equal ~msg:"params" ~cmp:cmp_params ~printer:Sql.show_params params
    (List.map (function Single p -> p | Choice _ -> assert false) stmt.vars);
  match kind with
  | Some k ->
      assert_equal ~msg:"kind" ~printer:[%derive.show: Stmt.kind] k stmt.kind
  | None -> ()
...
let wrong sql =
  sql >:: fun () ->
  ("Expected error in : " ^ sql)
  @?
  try
    ignore (Main.parse_one' (sql, []));
    false
  with _ -> true
...
let test =
  [
    ...
    tt "SELECT str FROM test WHERE id=?" [ attr "str" Text ] [ (param, Int) ];
    tt
      "SELECT x,y+? AS z FROM (SELECT id AS y,CONCAT(str,name) AS x FROM test \
       WHERE id=@id*2) ORDER BY x,x+z LIMIT @lim"
      [ attr "x" Text; attr "z" Int ]
      [ (param, Int); (named "id", Int); (named "lim", Int) ];
    tt
      "select test.name,other.name as other_name from test, test as other \
       where test.id=other.id + @delta"
    ...
    tt "insert into test values" [] [ p "id" Int; p "str" Text; p "name" Text ];
    tt "insert into test (str,name) values" [] [ p "str" Text; p "name" Text ];
    tt "insert into test values (2,'hello' || ' world',@name)" []
      [ p "name" Text ];
    tt "insert or replace into test values (2,?,?)" []
      [ (param, Text); (param, Text) ];
    tt "replace into test values (2,?,?)" [] [ (param, Text); (param, Text) ];
    tt
      "select str, case when id > @id then name when id < @id then 'qqq' else \
       @def end as q from test"
    ...
    (* check precedence of boolean and arithmetic operators *)
      tt "select str from test where id>=@id and id-@x<@id" [ attr "str" Text ]
...
let test2 =
  [
    ...
    tt
      "update test, (select * from test2) as x set str = x.str where \
       test.id=x.id"
      [] [];
    tt
      "update test, (select * from test2) as x set name = x.str where \
       test.id=x.id"
      [] [];
    tt
      "update test, (select * from test2) as x set test.str = x.str where \
       test.id=x.id"
      [] [];
    wrong
      "update test, (select * from test2) as x set test.name = x.name where \
       test.id=x.id";
    wrong
      "update test, (select * from test2) as x set test.str = str where \
       test.id=x.id";
  ]

let test3 =
  [
    tt "SELECT id FROM test WHERE str IN ( SELECT str FROM test2 )"
      [ attr "id" Int ] [];
...
let test4 =
  ...
  [
    ...
    tt
      "select least(10+unix_timestamp(),random()), \
       concat('test',upper('qqqq')) from test"
      [ attr "" Int; attr "" Text ]
      []
      ~kind:(Select `Nat);
    tt "select greatest(10,x) from test4" a [] ~kind:(Select `Nat);
    tt "select 1+2 from test4 where x=y" a [] ~kind:(Select `Nat);
    tt "select max(x) as q from test4 where y = x + @n" [ attr "q" Int ]
      [ (named "n", Int) ]
      ~kind:(Select `One);
    tt "select coalesce(max(x),0) as q from test4 where y = x + @n"
      [ attr "q" Int ]
      [ (named "n", Int) ]
      ~kind:(Select `One);
  ]
...
let test_parsing =
  [
    tt "CREATE TABLE test5_1 (x INT NOT NULL, y INT DEFAULT -1) ENGINE=MEMORY"
      [] [];
    tt "SELECT 2+3, 2+-3, -10 FROM test5_1"
      [ attr "" Int; attr "" Int; attr "" Int ]
      [];
  ]
...
let test_join_result_cols () =
  ...
  do_test "SELECT * FROM t1 NATURAL JOIN t2 WHERE j > @x"
    (ints [ "j"; "i"; "k" ])
    [ (named "x", Int) ];

  (*   NATURAL JOIN with common column qualified in WHERE *)
  do_test "SELECT * FROM t1 NATURAL JOIN t2 WHERE t2.j > @x"
    (ints [ "j"; "i"; "k" ])
    [ (named "x", Int) ];
  ()
...
let test_enum =
  [
    tt
      "CREATE TABLE test6 (x enum('true','false') COLLATE utf8_bin NOT NULL, \
       y INT DEFAULT 0) ENGINE=MyISAM DEFAULT CHARSET=utf8"
      [] [];
    tt "SELECT * FROM test6"
      [ attr "x" Text ~extra:[ NotNull ]; attr "y" Int ]
      [];
    tt "SELECT x, y+10 FROM test6"
      [ attr "x" Text ~extra:[ NotNull ]; attr "" Int ]
      [];
  ]
...
let run () =
  ...
  @@
  if
    List.exists (function RFailure _ | RError _ -> true | _ -> false) results
  then 1
  else 0
